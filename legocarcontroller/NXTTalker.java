...
public class NXTTalker {
    ...
    private synchronized void setState(int state) {
        mState = state;
        if (mHandler != null) {
            mHandler.obtainMessage(LEGOCarController.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
    ...
    private void toast(String text) {
        if (mHandler != null) {
            Message msg = mHandler.obtainMessage(LEGOCarController.MESSAGE_TOAST);
            Bundle bundle = new Bundle();
            bundle.putString(LEGOCarController.TOAST, text);
            msg.setData(bundle);
